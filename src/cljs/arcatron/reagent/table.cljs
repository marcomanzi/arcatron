...
(defn- pagination [table-id]
  [:ul.pagination.offset-md-5
   ...
   (if (not (s/last-page? table-id 100)) [page-item ">" (+ @(s/table-page table-id) 1) table-id])])

(defn paginated-table
  [{:keys [id headers elements element-key row-fields on-element-click-url] :as config}]
  (let [_ (s/table-page id 0)]
    [:span
